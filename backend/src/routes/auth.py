...
def login():
    ...
    if firebase_user_account_info["users"][0]["emailVerified"] == False:
        return {
            "message": "Verify your E-Mail via the link sent to your inbox before logging in (it may have landed in your spam folder)"
        }, 401
    ...
    if current_app.config.get("ENV") == "production":
        response.set_cookie(key="refreshToken", value=auth["refreshToken"], httponly=True, secure=True, samesite="None")
    else:
        response.set_cookie(key="refreshToken", value=auth["refreshToken"], httponly=True, secure=True, samesite="None")
    return response, 200
